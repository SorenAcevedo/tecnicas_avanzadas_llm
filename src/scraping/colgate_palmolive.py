    ...
    try:
        ...
        for a in soup.find_all("a", href=True):
            ...
            if parsed.netloc == urlparse(START_URL).netloc:
                if (
                    abs_link_no_fragment not in visited
                    and abs_link_no_fragment not in queue
                ):
...
def main():
    ...
    all_urls = [
        url for url in all_urls if ("palmolive" in url.lower() or "colgate" in url.lower())
    ]
    print(f"Processing {len(all_urls)} filtered URLs (palmolive/colgate only)")
    results = []
